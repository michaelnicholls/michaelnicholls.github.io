...
def getthisweekjson():
    ...
    if hana is None:
        ...
        cursor.execute(sql)
        conn.commit()  
        cursor.execute("select distinct country,location,course from event order by country,location,course")
        ...
        for row in result:
            ...
            prevCountry = country
            html+='{country:"'+thisCountry+'", location:"'+thisLocation+'", course:"'+course+'"},'
        html+="];"
        return html
@app.route('/thisweek')
def getthisweek():
